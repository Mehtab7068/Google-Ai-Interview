...
# # # Using gemini-2.5-flash as it is blazing fast and specialized in structured outputs
...
# #         # Offload API networking call to an executor to keep the server responsive
...
# #         # Clean up lists if the model bypassed instructions
# #         clean_questions = []
# #         for q in questions:
...
# #         # Enforcing native, structured JSON output matching our Pydantic response schema directly in Gemini
# #         loop = asyncio.get_event_loop()
# #         response = await loop.run_in_executor(
# #             None,
# #             lambda: client.models.generate_content(
...
# # The Client instance supports both sync and async (.aio) operations seamlessly
# client = genai.Client(api_key=api_key)
...
#         # NATIVE ASYNC CALL: Uses non-blocking asynchronous I/O natively via client.aio
...
#         # NATIVE ASYNC CALL WITH STRUCTURED JSON OUTPUT
#         response = await client.aio.models.generate_content(
#             model=GEMINI_MODEL_NAME,
#             contents=user_prompt,
#             config=types.GenerateContentConfig(
