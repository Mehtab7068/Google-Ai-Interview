...
import FormData from 'form-data';
import { requestAiService } from '../utils/aiClient.js';
import { pushSocketUpdate } from '../config/socket.js';
...
/**
 * Calculates overall and category averages for a completed session.
 */
const calculateOverallScore = async (sessionId) => {
    const session = await Session.findById(sessionId);
    ...
    const totalTech = evaluatedQuestions.reduce((sum, q) => sum + (q.technicalScore || 0), 0);
    const totalConf = evaluatedQuestions.reduce((sum, q) => sum + (q.confidenceScore || 0), 0);

    const avgTechnical = Math.round(totalTech / evaluatedQuestions.length);
    const avgConfidence = Math.round(totalConf / evaluatedQuestions.length);

    // Weighted Overall Score: 70% Technical + 30% Confidence
    const overallScore = Math.round(avgTechnical * 0.7 + avgConfidence * 0.3);

    return { overallScore, avgTechnical, avgConfidence };
};

/**
 * Async Worker Function to handle Transcription and Gemini Evaluation.
 * Runs in background after submission response is returned to client.
 */
const evaluateAnswerAsync = async (
    ...
    // --- Phase 1: Audio Transcription via FastAPI ---
    if (audioBuffer && audioBuffer.length > 0) {
        try {
            ...
            if (!transResponse.ok) {
                const errorText = await transResponse.text();
                throw new Error(`Transcription service HTTP ${transResponse.status}: ${errorText}`);
            }

            const transData = await transResponse.json();
            transcription = transData.transcription || "";

            console.log(`[Express] Received Transcription for Q${questionIdx + 1}: "${transcription}"`);
    ...
    // --- Phase 2: Gemini Evaluation via FastAPI ---
    try {
        pushSocketUpdate(io, userId, sessionId, 'AI_EVALUATING', `AI is evaluating answer for Q${questionIdx + 1}...`);
        ...
        if (!evalResponse.ok) {
            const pythonErrorText = await evalResponse.text();
            throw new Error(`AI Evaluation Service HTTP ${evalResponse.status}: ${pythonErrorText}`);
        }

        const evalData = await evalResponse.json();

        // Update question evaluation details
        question.userAnswerText = transcription;
        question.userSubmittedCode = code || "";
        question.technicalScore = evalData.technicalScore;
        question.confidenceScore = evalData.confidenceScore;
        ...
        if (session.status === 'completed' || allQuestionsEvaluated) {
            ...
            await session.save();
            pushSocketUpdate(io, userId, sessionId, 'SESSION_COMPLETED', 'Session evaluation complete and scores finalized.', session);
        } else {
            await session.save();
            pushSocketUpdate(io, userId, sessionId, 'EVALUATION_COMPLETE', `Evaluation ready for Q${questionIdx + 1}`, session);
        }

    } catch (error) {
        console.error(`❌ Evaluation Processing Error for Q${questionIdx + 1}: ${error.message}`);
        pushSocketUpdate(io, userId, sessionId, 'EVALUATION_ERROR', `Failed to evaluate Q${questionIdx + 1}.`);
    }
};
...
export const startSession = async (req, res) => {
    try {
        ...
        if (!role || !level) {
            res.status(400);
            throw new Error('Role and Level are required to start a session.');
        }

        // Fetch questions matching role and level
        let questions = await QuestionBank.aggregate([
            { $match: { role: role, level: level } },
            { $sample: { size: parseInt(questionCount, 10) } },
        ]);

        // Fallback if specific pool is sparse
        ...
        if (questions.length === 0) {
            res.status(404);
            throw new Error(`No questions found for Role: ${role} and Level: ${level}.`);
        }

        const sessionQuestions = questions.map((q) => ({
            questionId: q._id,
            questionText: q.questionText,
            questionType: q.questionType || 'verbal',
            isSubmitted: false,
            isEvaluated: false,
        }));
...
/**
 * @desc    Submit an answer for a specific question (Audio buffer or Code)
...
export const submitAnswer = async (req, res) => {
    try {
        ...
        const questionIdx = parseInt(questionIndex, 10);
        if (isNaN(questionIdx) || questionIdx < 0 || questionIdx >= session.questions.length) {
            res.status(400);
            throw new Error('Invalid question index provided.');
        }
        ...
        // Extract memory buffer provided by Multer
        if (req.file) {
            console.log(`[Express] Audio File Received in RAM. Size: ${req.file.size} bytes, MIME: ${req.file.mimetype}`);

            if (req.file.size === 0) {
                res.status(400);
                throw new Error('Uploaded audio recording is empty (0 bytes). Please record again.');
            }
        ...
        if (!audioBuffer && !codeSubmission) {
            res.status(400);
            throw new Error('Submission must contain either an audio recording or code.');
        }
        ...
        res.status(202).json({
            success: true,
            message: 'Answer received. Evaluation processing asynchronously.',
            status: 'received',
...
/**
 * @desc    Get all historical sessions for logged-in user
...
/**
 * @desc    End an interview session manually
